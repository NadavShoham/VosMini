...
def test():
    ...
    video_path = 'data/videos/dog.mp4'
    video_capture = cv2.VideoCapture(video_path)

    # turn video into frames
    frames_dir = 'data/frames/dog'
    ...
    output_path_fg = 'data/images/dog/object.png'
    cv2.imwrite(output_path_fg, object_frame)
    output_path_bg = 'data/images/dog/background.png'
    cv2.imwrite(output_path_bg, background_frame)

    object_frame = cv2.imread(output_path_fg)
    background_frame = cv2.imread(output_path_bg)

    # preprocess each frame
    obj_data, dims = pre_process_image(object_frame)
    bgd_data, dims = pre_process_image(background_frame)

    prior = DPMMPython.create_prior(5, 0, 1, 10, 1)
    ...
    new_frames_dir = 'data/new_frames/dog'
    ...
    while True:
        ...
        new_frame = cv2.addWeighted(paint_object, 0.2, frame, 0.8, 0)
        ...
        # use fit_partial with new data
        updated_object_frame_data = np.reshape(updated_object_frame_data, (5, len(updated_object_frame_data)))
        updated_bgd_frame_data = np.reshape(updated_bgd_frame_data, (5, len(updated_bgd_frame_data)))
        obj_model = DPMMPython.fit_partial(obj_model, iterations=1, t=frame_count, data=updated_object_frame_data)
        bgd_model = DPMMPython.fit_partial(bgd_model, iterations=1, t=frame_count, data=updated_bgd_frame_data)
